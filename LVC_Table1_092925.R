...
comor_earlypvi <- dbGetQuery(conn, "SELECT  
                             ...
                             MAX(
                               ...
                               WHEN a.condition_concept_id IN (46270483,4196141,4175440,37016767,37016768,376979,4225656,4221495,43531578,4338897,442793,45769832,43531616,201820,4008576,4033942,4159742,4009303,4048028,4042502,443767,42538169,443733,192279,443730,
                                 ...
                                 AND a.condition_start_date >= (b.cohort_start_date - INTERVAL 365 DAY) AND a.condition_start_date <  b.cohort_start_date
                               THEN 1 
                               ELSE 0 
                               END
                             ) AS diabetes,
                             ...
                             MAX(
                               ...
                               WHEN (
                                 (o.observation_concept_id IN (4005823, 4276526, 4218741) 
                                  AND o.observation_date >= (b.cohort_start_date - INTERVAL 365 DAY) AND o.observation_date <  b.cohort_start_date)
                                 OR
                             ...
                             WHERE 
                             b.cohort_definition_id = 664 --CHANGE THIS: Replace 664 with your Early PVI ID 
                             GROUP BY 
...
ON 
    b.subject_id = o.person_id
WHERE 
    b.cohort_definition_id = 665 --CHANGE THIS: Replace 665 with your Non-Early PVI ID
GROUP BY 
    b.subject_id, b.cohort_start_date
")
...
table1_df <- mytable(group ~ age + gender_concept_id + race_concept_id + ethnicity_concept_id +
                          diabetes + hypertension + smoking + eskd + copd + 
                          chronic_kidney_disease + ischemic_heart_disease + 
                          congestive_heart_failure, 
                        data = filtered_data)
